...
export default function App() {
  ...
  const getData=(page)=>{
    fetch(`http://localhost:8000/employeedetails?_limit=5&_page=${page}`)
      .then(res => res.json())
      .then((res) => {
        setData([...res]);
      })
  ...
  const handleTask = async ({ name, age, address, department, salary, maritalStatus, image }) => {
    ...
    await fetch(`http://localhost:8000/employeedetails`, config)
  }

  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:8000/employeedetails/${id}`, {
        method: "DELETE"
      })
    }
    catch (err) {
  ...
  return (
    <div className="App">
      ...
      <div className="dept">
        Department: {" "}
        {
          ["Show All","HR", "Engineering", "Design", "Planning"].map((dept)=>{
      ...
      <Display handleDelete={handleDelete} data={data.filter((department)=>{
